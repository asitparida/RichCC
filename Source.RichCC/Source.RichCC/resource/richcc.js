/* FOCUSABLE SELECTOR FRM JQUERY UI */
(function (factory) {
...
(function ($) {    
    // Selectors
    console.log($.getfocusable);
    if (!$.getfocusable) {
        $.getfocusable = function (element, hasTabindex) {
        ...
        $.extend($.expr[":"], {
            getfocusable: function (element) {
                return $.getfocusable(element, $.attr(element, "tabindex") != null);
            }
        });
                                        ...
                                        if (_popUpElm) {
                                            ...
                                            $(_popUpElm).on(evtKeyUpId, function (e) {
                                                if (e.keyCode == 27) {
                                                    ...
                                                    var _lastFocusable = $('#popup_' + column.key).find(':getfocusable').last();
                                                    if (_lastFocusable && e.target) {
                                                        if ($(_lastFocusable).get(0).isEqualNode($(e.target).get(0)) == true) {
                                                            e.stopPropagation();
                                                            e.preventDefault();
                                                        }
                                                    }
                                                    ...
                                                    var _lastFocusable = $('#popup_' + column.key).parent().find(':getfocusable').first();
                                                    if (_lastFocusable && e.target) {
                                                        if ($(_lastFocusable).get(0).isEqualNode($(e.target).get(0)) == true) {
                                                            e.stopPropagation();
                                                            e.preventDefault();
